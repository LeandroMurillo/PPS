...
- *Agrupaciones / Colectivos*: Entidades propias (ej. bandas, colectivos de producción artesanal) que poseen un nombre, género o rama productiva y un historial de eventos. Están conformadas por múltiples integrantes. El usuario que inscribe a la agrupación funge como administrador principal del perfil de la misma (`UsuarioDueño`), y puede cargar a los demás integrantes mediante DNI o correo. El sistema los vinculará automáticamente a la agrupación si están registrados en la plataforma. //, o les enviará una invitación de registro si aún no lo están.
...
- *Supuestos de la Base de Datos*:
  - _Formularios Dinámicos (Patrón EAV)_: Se asume un modelo de "Entidad-Atributo-Valor" apoyado en el uso de campos JSON (`valor` en la tabla `RESPUESTASFORMULARIOS`) para permitir que cada Categoría posea preguntas y esquemas de datos disímiles sin alterar el esquema relacional rígido.
  - _Propiedad y Pertenencia_: El modelo asume que todo Actor Cultural posee un creador inicial de carácter obligatorio (representado por `idUsuarioDueño`), complementado por la tabla asociativa `INTEGRANTES` que gestiona al resto de los participantes de un colectivo.
