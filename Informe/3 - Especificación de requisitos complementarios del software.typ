...
Para registrarse en el sitio web, primero, los usuarios deberán completar los siguientes datos obligatorios: correo electrónico, contraseña, apellidos, nombres, DNI, fecha de nacimiento y género, además, deberán enviar una imagen de un documento de identidad. Es importante que el usuario confirme estos datos antes de avanzar al siguiente paso de registro. La dirección de correo electrónico es obligatoria y única. La contraseña deberá tener una longitud mínima de 6 caracteres. Finalmente, se le enviará un correo electrónico a la dirección provista para activar la cuenta de usuario.

Una vez que un usuario se encuentre activo y habilitado para iniciar sesión, podrá crear su actor cultural (completando el formulario específico de su sector y bajo la restricción de un único formulario en validación), armar su portafolio, editar sus datos, integrar agrupaciones, buscar y postular a eventos vigentes en la plataforma, y ver los datos públicos.
...
- *Agrupaciones / Colectivos*: Entidades propias (ej. bandas, colectivos de producción artesanal) que poseen un nombre, género o rama productiva y un historial de eventos. Están conformadas por múltiples integrantes. El usuario que inscribe a la agrupación funge como administrador del perfil de la misma, y puede cargar a los demás integrantes mediante DNI o correo. El sistema los vinculará automáticamente a la agrupación si están registrados en la plataforma. //, o les enviará una invitación de registro si aún no lo están.
...
- *Requisitos tecnológicos*: La aplicación seguirá una arquitectura Cliente/Servidor sobre Internet.
  - *Servidor*: Deberá estar configurado para soportar el entorno de ejecución Django y un motor de base de datos, dimensionado para gestionar múltiples conexiones concurrentes (especialmente ante picos de visitas en relevamientos).
...
- *Tiempo de respuesta*: Las operaciones de lectura (listado de actores, categorías, portafolios) no deberán superar los 3 segundos bajo condiciones normales de red. Las operaciones de escritura (carga de imágenes, creación de actores y vinculación de integrantes) no deberán superar los 10 segundos, dependiendo del ancho de banda del usuario para la subida de archivos.
- *Concurrencia*: El sistema debe ser capaz de soportar múltiples personas accediendo simultáneamente a los datos públicos, así como registros simultáneos durante periodos de convocatorias o censos de relevamiento, sin degradar la experiencia de navegación.
...
- *Ajuste a estándares*: La especificación de requisitos se basa en el estándar IEEE 830. El desarrollo del código sigue estándares modernos de JavaScript y patrones de diseño estructurados (como MVC/MTV).
- *Seguridad*:
  - Las contraseñas se almacenarán encriptadas utilizando algoritmos de _hash_ robustos (Bcrypt).
...
- *Política de Respaldo*: Se establece una política de respaldo completo mensual de la base de datos y de los archivos almacenados. //, complementada con respaldos incrementales diarios.
- *Política de Borrado*: Se implementará una política de borrado lógico para entidades críticas. Por ejemplo, las categorías no se eliminarán físicamente de la base de datos si tienen actores asociados, sino que cambiarán su estado a "DadoDeBaja" para mantener la integridad histórica de los datos.
