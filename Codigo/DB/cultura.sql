...
CREATE TABLE IF NOT EXISTS `cultura`.`FormulariosSubcategoria` (
  ...
  `esPublico` TINYINT NOT NULL,
  `estado` ENUM('A', 'I') NOT NULL,
  PRIMARY KEY (`idCampo`, `idSubcategoria`),
  INDEX `FK_CamposFormularios_idCampo1_idx` (`idCampo` ASC) VISIBLE,
  INDEX `fk_FormulariosSubcategoria_Subcategorias1_idx` (`idSubcategoria` ASC) VISIBLE,
  CONSTRAINT `FK_CamposFormularios_idCampo1`
    FOREIGN KEY (`idCampo`)
    REFERENCES `cultura`.`CamposFormulario` (`idCampo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_FormulariosSubcategoria_Subcategorias1`
    FOREIGN KEY (`idSubcategoria`)
    REFERENCES `cultura`.`Subcategorias` (`idSubcategoria`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
